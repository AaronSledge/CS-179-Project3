...
def inSet(matrix, open_set):
    while(len(open_set) != 0):
        _, _, _, curr_matrix = heapq.heappop(open_set)
        if(curr_matrix == matrix):
            return True
    return False

def addToSetAstar(curr_matrix, new_matrix, container, cost, actionList, open_set, child, row, col, tieBreak, maxActions):
    ...
    heapq.heappush(open_set, (fn, gn, next(tieBreak), new_matrix))
